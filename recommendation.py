...
def calculate_edge_weight(track_u, track_v, weights):
    ...
    try:
        ...
        # to penalize a bit large sets
        genre_sim = len(genre_intersection) if len(genre_u) == len(genre_v) else len(genre_intersection) - 0.5*(max(len(genre_u), len(genre_v)) - len(genre_intersection))
        """if len(track_u['genre']) == 1:
            feature_weight = weights['feature'] * 2
        else :"""
    ...
    #topic_sim = topic_intersection / (len(topic_u) + len(topic_v) - topic_intersection)
    topic_sim = topic_intersection
    topic_weight = weights['topic']

    # Feature similarity (using Jaccard-like similarity)
    ...
    #feature_sim = feature_intersection / (len(feature_u) + len(feature_v) - feature_intersection)
...
def hybrid_recommendation_system(initial_query_id, tracks_data, weights, max_playlist_size=100, max_songs_per_artist=1):
    """
    ...
    safe_artist_name = "".join(c if c.isalnum() or c in (" ", "-", "_") else "" for c in initial_artist).replace(" ", "_")
    safe_track_name = "".join(c if c.isalnum() or c in (" ", "-", "_") else "" for c in initial_song).replace(" ", "_")
    output_file = f'generated_playlist_from_{safe_artist_name}_{safe_track_name}.csv'
...
file_path = 'final_processed_music_data.csv'
